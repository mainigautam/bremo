...
const MemoLeafImage = ({ d, i, fetchData, delPath,handleModal,setModalData,setLoader}) => {
    const footer = (fut) => {
    ...
    return (
        <div className="memoLeafContainer" onMouseEnter={(e) => { footer(i) }} onMouseLeave={(e) => { removeFooter(i) }} style={{backgroundColor:d.theme}}> 
            <div className="memoLeafImageHolder">
                {d.image === undefined ? null : <img src={new Buffer.from(d.image).toString("ascii")} alt="" className="memoLeafImage" onClick={(e)=>{handleModal();setModalData(d);}}/>}
            </div>
